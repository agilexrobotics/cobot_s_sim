...
bool DelAllTraj_Callback(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
{
    std::string info;
    bool ret = Del_All_Traj();
    if (ret)
    {
        info = "Delete all trajectory success!\n";
        ROS_INFO_STREAM(info.c_str());
    }
    else
    {
        info = "Delete all trajectory failed!\n";
        ROS_ERROR_STREAM(info.c_str());
    }
    res.success = ret;
    res.message = info;
    return ret;
...
void timer_callback(const ros::TimerEvent)
{
    ...
    if(timer_cnt > 1)
    {
        timer_cnt = 0;
        Get_Joint_Err_Flag();
        // ROS_INFO("Get_Joint_Err_Flag");
    }
    else
    {
        Get_Arm_Joint();
        // ROS_INFO("Get_Arm_Joint");
    }
}
...
int main(int argc, char **argv)
{
    ...
    //service
    DelAllTraj_ = nh_.advertiseService("/rm_driver/Del_All_Traj", DelAllTraj_Callback);
    ...
    while(ros::ok())
    {
        while(1)
        {
            ...
            // ROS_INFO("socket recv a frame: %s", socket_buffer);
            buffer_cnt++;
