...
{
    ros::init(argc, argv, "rm75_pick_place");
    ros::NodeHandle nh;
...
//    pose.orientation.w = 1.0;
        ...
        attached_object.touch_links = std::vector<std::string>{"Link7"};
...
//    target_pose2.position.y = 0.394717;
//    target_pose2.position.z = 0.504879;
        group.setPoseTarget(target_pose2, "Link7");
        ...
        if(success2)
        {
            ...
            //创建一个用于解除附加到机械臂Link7的附加物体
            moveit_msgs::AttachedCollisionObject detach_object;
            detach_object.link_name = "Link7";
            /* The header must contain a valid TF frame*/
